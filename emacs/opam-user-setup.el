...
;; ## added by OPAM user-setup for emacs / tuareg ## 17c5423cd9f9090295b8a41208045c90 ## you can edit, but keep this line
...
(defun opam-tuareg-autoload (fct file doc args)
  (let ((load-path (cons (expand-file-name "~/.opam/default/share/emacs/site-lisp") load-path)))
    (load file))
  (apply fct args))
...
(when (not (assoc "ocp-indent" opam-tools-installed))
  (autoload 'ocp-setup-indent (expand-file-name "~/.opam/default/share/emacs/site-lisp/ocp-indent.el") "Improved indentation for Tuareg mode")
  (autoload 'ocp-indent-caml-mode-setup (expand-file-name "~/.opam/default/share/emacs/site-lisp/ocp-indent.el") "Improved indentation for Caml mode")
  (add-hook 'tuareg-mode-hook 'ocp-setup-indent t)
  (add-hook 'caml-mode-hook 'ocp-indent-caml-mode-setup  t)
  (setq ocp-indent-path(expand-file-name "~/.opam/default/bin/ocp-indent"))
